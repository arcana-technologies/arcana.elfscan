...
bool
ac_internal_elf_dyn_entry(elfobj_t *elfobj, uint16_t tag,
    struct elf_dynamic_entry ***dyn, size_t *retcount)
{
	...
	*retcount = 0;
	...
	printf("dyn: %p\n", dyn);
	printf("Setting *dyn: %p\n", *dyn);
	
	elf_dynamic_iterator_init(elfobj, &iter);
	for (;;) {
		size_t sz = sizeof(struct elf_dynamic_entry);
		elf_iterator_res_t res =
		    elf_dynamic_iterator_next(&iter, &entry);
		if (res == ELF_ITER_ERROR) {
			ac_printf("elf_dynamic_iterator_next failed\n");
			return false;
		} else if (res == ELF_ITER_DONE) {
			return true;
		} else if (res == ELF_ITER_OK) {
			if (entry.tag == tag) {
				if (*retcount >= 1) {
					sz = sz + sizeof(struct elf_dynamic_entry);
					**dyn = realloc(**dyn, sz);
					if (**dyn == NULL) {
						perror("realloc");
						return false;
					...
					**dyn = malloc(sizeof(struct elf_dynamic_entry));
					printf("**dyn after malloc: %p\n", **dyn);
				}
				memcpy(*((*dyn) + *retcount), &entry, sizeof(entry));
				*retcount += 1;
				break;
			}
		}
